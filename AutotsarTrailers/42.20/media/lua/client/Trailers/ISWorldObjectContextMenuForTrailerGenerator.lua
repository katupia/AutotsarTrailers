...
ISWorldObjectContextMenuForTrailerGenerator.generatorPlug = function(playerObj, trailer)
	if luautils.walkAdj(playerObj, trailer:getSquare()) then
		ISTimedActionQueue.add(ISPlugTrailerGenerator:new(playerObj, trailer));
	end
end

function ISWorldObjectContextMenuForTrailerGenerator.generatorUnplug(playerObj, trailer)
	if luautils.walkAdj(playerObj, trailer:getSquare()) then
		ISTimedActionQueue.add(ISUnplugTrailerGenerator:new(playerObj, trailer));
	end
end

ISWorldObjectContextMenu.generatorTrailerClear = function(player, context, worldobjects, test)
    --print("generatorTrailerClear")
    local fetch = ISWorldObjectContextMenu.fetchVars
    if fetch and fetch.generator then
        --print("generatorTrailerClear has generator fetch")
        local md = fetch.generator:getModData()
        local generatorFullType = md.generatorFullType
        if generatorFullType and generatorFullType == "Autotsar.InvisibleGenerator" then
            --print("generatorTrailerClear has invisible generator fetch")
            context:removeOptionByName(getText("ContextMenu_Generator"))--remove submenu
        end
    end
...
ISWorldObjectContextMenuForTrailerGenerator.clearGeneratorMenu = function(context)
	...
	context:removeOptionByName(getText("ContextMenu_GeneratorPlugTT"))
	context:removeOptionByName(getText("ContextMenu_GeneratorFixTT"))
...
ISWorldObjectContextMenuForTrailerGenerator.onToggleSpotlights = function(playerObj, trailer)
    if luautils.walkAdj(playerObj, trailer:getSquare()) then
        ISTimedActionQueue.add(ISToggleSpotlights:new(playerObj, trailer));
    end
end
...
ISWorldObjectContextMenuForTrailerGenerator.onActivateGenerator = function(player, trailer, generator, enable)
	...
	if luautils.walkAdj(playerObj, generator:getSquare()) then
		ISTimedActionQueue.add(ISActivateTrailerGenerator:new(playerObj, trailer, generator, enable));
	end
end
		...
		-- if generator then
			...
			if old_options then 
				...
				if not generator:isActivated() then
					...
					context:removeOptionTsar(context:getOptionFromName(getText("ContextMenu_Generator")))
